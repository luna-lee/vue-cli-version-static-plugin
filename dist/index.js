            ...
            compiler.options.plugins.forEach((pluginClass) => {
                ...
                if (pluginClass.constructor.name == 'HtmlWebpackPlugin' && isProd) {
                    ...
                    if (this.option.dynamicPublicPath) {
                        ...
                        let httpCdn = {};
                        cdnStatic = Object.keys(cdnObj).reduce(
                            (obj, key) => {
                                cdnObj[key].forEach((item) => {
                                    if (reg.test(JSON.stringify(item))) {
                                        item = JSON.parse(JSON.stringify(item).replace(reg, process.env.JsStaticReplaceDir));
                                        if (!obj[key]) obj[key] = [];
                                        if (key == 'css') {
                                            obj[key].push({
                                                tagName: 'link',
                        ...
                        options.cdn = JSON.parse(JSON.stringify(options.cdn).replace(reg, process.env.JsStaticReplaceDir));
                    }
                }
            });
        });
...
//生成sourceMap文件
function saveFile(outputDir, assets, loadMethod) {
